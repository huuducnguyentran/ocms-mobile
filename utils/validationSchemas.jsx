  ...
  },
  {
    validator: (_, value) => {
      ...
      if (cleaned.length !== 12) {
        return Promise.reject(new Error('Citizen ID must contain exactly 12 digits (no spaces or special characters)'));
      }
      return Promise.resolve();
    },
  },
 ...
 */
export const createScoreValidation = (label, required = false) => {
  const rules = [];
  ...
  rules.push({
    validator: (_, value) => {
      ...
      if (strValue.includes(',')) {
        return Promise.reject(new Error('Please use dot (.) instead of comma (,) for decimal numbers'));
      }
 ...
 * @returns {array} - Validation rules
 */
export const createPercentageValidation = (label, required = false) => {
  const rules = [];
...
export const minAttendanceValidation = createScoreValidation('Minimum Attendance', false);
export const minPracticeExamScoreValidation = createScoreValidation('Minimum Practice Exam Score', false);
export const minFinalExamScoreValidation = createScoreValidation('Minimum Final Exam Score', true);
export const minTotalScoreValidation = createScoreValidation('Minimum Total Score', true);
 ...
 */
export const formatScoreInput = (value) => {
 ...
 */
export const validateGradeInput = (value, fieldName = 'Grade') => {
 ...
 */
export const validateGradeEntry = (gradeEntry) => {
  const errors = [];

  // Validate attendance
  if (gradeEntry.attendance !== null && gradeEntry.attendance !== undefined && gradeEntry.attendance !== '') {
  ...
  if (gradeEntry.practiceExamScore !== null && gradeEntry.practiceExamScore !== undefined && gradeEntry.practiceExamScore !== '') {
    const result = validateGradeInput(gradeEntry.practiceExamScore, 'Practice Exam Score');
    if (!result.isValid) {
      errors.push(result.error);
    }
  }
  ...
  if (gradeEntry.finalExamScore !== null && gradeEntry.finalExamScore !== undefined && gradeEntry.finalExamScore !== '') {
    const result = validateGradeInput(gradeEntry.finalExamScore, 'Final Exam Score');
    if (!result.isValid) {
      errors.push(result.error);
    }
  }
  ...
  if (gradeEntry.totalScore !== null && gradeEntry.totalScore !== undefined && gradeEntry.totalScore !== '') {
 ...
 */
export const createEndDateValidation = (getFieldValue) => [
  {
    required: true,
  ...
  },
  {
    validator: (_, value) => {
      ...
      if (startDate && selectedDate.isSame(dayjs(startDate).startOf('day'), 'day')) {
        return Promise.reject(new Error('End date must be after start date'));
      }
 ...
 */
export const createEditStartDateValidation = (originalStartDate) => [
  {
    required: true,
    message: 'Please select start date',
  },
  {
    validator: (_, value) => {
      ...
      const selectedDate = dayjs(value).startOf('day');
      const today = dayjs().startOf('day');
      const originalDate = originalStartDate ? dayjs(originalStartDate).startOf('day') : null;
 ...
 */
export const createDisablePastDatesExcept = (allowedDate) => (current) => {
  if (!current) return false;
  const today = dayjs().startOf('day');
